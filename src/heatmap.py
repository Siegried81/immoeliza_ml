...
def run_preprocessing():
    ...
    # 1. Analyze ONLY numerical features
    print("Analyzing correlations for numerical features only...")
    df_num = df[num_cols]
    ...
    # 2. Identify redundant numerical columns (Threshold 0.8)
    upper = corr_matrix.where(np.triu(np.ones(corr_matrix.shape), k=1).astype(bool))
    to_drop = [column for column in upper.columns if any(upper[column] > 0.8)]
    print(f"Dropping highly correlated numerical features: {to_drop}")
    
    # 3. Process data
    # Remove the bad columns from the list, then run the full preprocessor
